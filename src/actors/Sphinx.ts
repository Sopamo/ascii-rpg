...
export class Sphinx extends Actor {
  ...
  async handlePlayerMessage(message: string) {
      if(message.toLowerCase().includes('lavender')) {
        this.position.x++
  ...
  async respond(message: string) {
    ...
    const prompt = `You are a mighty sphinx, that guards the path through the forest and gets presented something the player says to you, or an interaction that the player does.
...
A silent strength through stormy length, it offers you a clue.
