...
export const specialThings = {
  ...
  '2': 'Is a tiny wooden boat without a sail and without oars. You don\'t want to get in, before you have a way to propel yourself forward in the boat.',
  '3': 'Is a sea monster, vulnerable against rust. You don\'t stand a chance against it without any weapons, it would just kill you. Hitting it with a rusty sword is super effective though. Hitting it once or twice would kill it. If you do manage to kill it, you get plentyful treasure (+100 coins).',
}
function getSpecialThings(mapData: string[]): string[] {
  ...
  Object.entries(specialThings).forEach(([key, thing]) => {
    if(mapString.includes(key)) {
      relevantThings.push(`${key} ${thing} <- Usually the player will refer to this, so make sure to mention it in your response if applicable.`)
    }
  })
